...
fun WriteLogScreen(
    ...
    LaunchedEffect(Unit) {
        try {
            writeLogModel.list(context, id.toInt())
        }catch (_:Exception){
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            itemsIndexed(writeLogModel.categoryAppTaskList){index,item->
               ListItem(leadingContent = {
                   AsyncImage(model = "${item.appLogo}", contentDescription = "")
               }, headlineContent = { Text(text = item.appName) }, trailingContent = {
                   Button(onClick = { /*TODO*/ }) {
                       Text(text = "详情")
                   }
               }, supportingContent = {
                   if("${item.taskEndDate}"!="null"){
                       Text(text = "${item.taskEndDate}")
                   }
               })
            }
        }
